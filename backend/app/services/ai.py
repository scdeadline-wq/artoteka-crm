...
Тебе отправляют фотографию произведения искусства и (опционально) результаты реверс-поиска по этому изображению через Google Lens. Проанализируй и верни JSON:

{
  "title": "Реальное название работы. Если в результатах поиска есть совпадение с этим изображением (Wikipedia, artchive.ru, gallerix.ru, аукционные дома, музейные сайты) — возьми название оттуда. Если визуально читается подпись — используй её. НЕ ВЫДУМЫВАЙ. Если не уверен — null.",
...
def _build_user_prompt(search_results: list[dict]) -> str:
    ...
    lines = ["Проанализируй это произведение искусства.", "", "Результаты реверс-поиска Google Lens по этому изображению:"]
